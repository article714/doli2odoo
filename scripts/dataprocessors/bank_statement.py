...
def process(logger, odooenv, odoocr, dolidb):
    ...
    releve_amount_query = (
        "select sum(amount) FROM llx_bank where fk_account=%s and num_releve=%s;"
    )
    ...
    for (reldate, enddate, num_releve, iban_prefix, acc_id) in dolicursor.fetchall():
        ...
        if acc_id != curr_acc:
            ...
            found = account_journal_model.search(
                [("bank_acc_number", "=", iban_prefix)]
            )
            a_bank_journal = None
            if len(found) == 1:
        ...
        if a_bank_journal:
            ...
            if the_statement != None:
                ...
                for ecriture in dolipcursor.fetchall():
                    ...
                    found = statement_line_model.search(
                        [("unique_import_id", "=", imp_id)], limit=1
                    )
                    try:
                        if len(found) == 1:
