...
def update_factures(default_product, logger, odooenv, odoocr, dolidb):
    ...
    try:
        ...
        tva_20 = tvas[0]
        tvas = odooenv['account.tax'].search(['&', ('description', '=', u'ACH-19.6'), ('active', '=', False)])
        if len(tvas) != 1:
            tva_196 = tva_20.copy()
            tva_196.write({'name':u'TVA déductible (achat) 19,6%', 'amount':19.6000,
                           'description':'ACH-19.6', 'active':False, 'tag_ids':(5, False, False)})
        else:
            tva_196 = tvas[0]
        ...
        dolicursor.execute("""SELECT f.rowid,f.ref,f.ref_supplier,f.datec,s.nom , t.libelle
                             FROM llx_societe s, llx_facture_fourn f
                               LEFT OUTER JOIN llx_c_payment_term t on f.fk_cond_reglement = t.rowid
                             WHERE f.fk_soc=s.rowid ;""")

        nestedquery = ("SELECT f.label,f.description,f.tva_tx,f.qty,f.pu_ht, p.ref FROM llx_facture_fourn_det f" +
        ...
        for (fac_id, facnum, ref_fourn, date_crea, soc_nom, cond_pai) in dolicursor.fetchall():
            ...
            if len(p_found) == 1:
                values = {'name':facnum, 'partner_id':p_found[0].id, 'number':facnum,
            ...
            if len(found) == 1 and len(p_found) == 1:
                ...
                logger.warn("WARNING: several account_invoice found for name = " + facnum)
            ...
            if fact != None:
                ...
                if dolipcursor:
                    for (label, description, tva_tx, qty, subprice, p_ref) in dolipcursor:
                        lf_found = account_invoice_line_model.search(['&', ('invoice_id', '=', fact.id), ('name', '=', description)])
                        nb_lf = len(lf_found)
                        if nb_lf < 2:
                            ...
                            values = {'invoice_id':fact.id, 'product_id':p_id, 'account_id':acc_id,
                                      'quantity':qty,
                        ...
                        if nb_lf == 1:
                            ...
                            logger.warn("WARNING: several account_invoice_line found for name = " + description)
