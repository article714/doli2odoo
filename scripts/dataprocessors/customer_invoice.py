...
def update_factures(default_product, logger, odooenv, odoocr, dolidb):
    ...
    try:
        ...
        tvas = odooenv['account.tax'].search([('name', '=', u'TVA collectée (vente) 20,0%')])
        tva_20 = tvas[0]
        tvas = odooenv['account.tax'].search(['&', ('description', '=', '19.6'), ('active', '=', False)])
        if len(tvas) != 1:
            tva_196 = tva_20.copy()
            tva_196.write({'name':u'TVA collectée (vente) 19,6%', 'amount':19.6000,
                           'description':'19.6', 'active':False, 'tag_ids':(5, False, False)})
        else:
            tva_196 = tvas[0]
        ...
        dolicursor.execute(""" SELECT f.rowid, f.facnumber,f.datec,f.date_valid,s.nom, t.libelle
                               FROM llx_societe s, llx_facture f
                               LEFT OUTER JOIN llx_c_payment_term t on f.fk_cond_reglement = t.rowid
                               WHERE f.fk_soc=s.rowid;""")

        nestedquery = ("SELECT f.label,f.description,f.tva_tx,f.qty,f.subprice, p.ref FROM llx_facturedet f" +
                        " LEFT OUTER JOIN llx_product p ON f.fk_product = p.rowid where f.fk_facture=%s")

        for (fac_id, facnum, date_crea, date_valid, soc_nom, cond_pai) in dolicursor.fetchall():
            ...
            if len(p_found) == 1:
                ...
                if len(found) == 1 :
                    ...
                    logger.warn("WARNING: several sale_order found for name = " + facnum)
            else:
            ...
            if sale_order != None:
                ...
                if dolipcursor:
                    for (label, description, tva_tx, qty, subprice, p_ref) in dolipcursor:
                        ol_found = sale_order_line_model.search(['&', ('order_id', '=', sale_order.id), ('name', '=', description)])
                        nb_ol = len(ol_found)
                        if nb_ol < 2:
                            p_id = default_product.id
                            if p_ref != None:
                                prod_found = product_template_model.search([('default_code', '=', p_ref)])
                                if len(prod_found) == 1:
                            ...
                            values = {'order_id':sale_order.id, 'product_id':p_id,
                                      'product_uom_qty':qty, 'customer_lead':0,
                                      'price_unit':subprice, 'name':description,
                                      'tax_id':taxes
                                      }
                        if nb_ol == 1:
                            ol = ol_found[0]
                            ...
                            logger.warn("WARNING: several sale_order_line found for name = " + description)
